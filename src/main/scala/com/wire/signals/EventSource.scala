...
/** A source of events of the type `E` which [[Subscriber]]s can attach themselves too,
...
trait EventSource[E] {
  ...
  /** Creates a [[Subscription]] to a [[Subscriber]] which will consume events in the given [[ExecutionContext]].
    ...
    *
    * @param executionContext An [[ExecutionContext]] in which the [[Subscriber]] function will be executed.
    * @param subscriber [[Subscriber]] - a function which consumes the event
    * @param eventContext an [[EventContext]] which will register the [[Subscription]] for further management (optional)
    * @return a [[Subscription]] representing the created connection between the [[EventSource]] and the [[Subscriber]]
    */
  ...
  /** Creates a [[Subscription]] to a [[Subscriber]] which will consume events in the same [[ExecutionContext]] as
  ...
  /** Creates a [[Subscription]] for the given [[Subscriber]].
    ...
    *
    * @param subscriber a [[Subscriber]] function which will be called every time an event is emitted
    * @param executionContext an [[ExecutionContext]] used for executing the [[Subscriber]] function (or `None` for the current one)
    * @param eventContext an [[EventContext]] which will register the [[Subscription]] for further management (optional)
    * @return
    */
...
/** [[Subscription]]s created for a [[ForcedEventSource]] cannot be unsubscribed.
  ...
  * (i.e. the [[Subscriber]] will stop receiving events until the consecutive call to [[Subscription.subscribe]])
  * but
  * {{{
